# Adding a method to byte-identity
...
`plugin.yaml`, `byte-identity.proto`, and `main.go`.
...
spec:
  ...
  - name: SendMessage
    ...
    request_proto:  "gibson.plugins.byte-identity.v1.SendMessageRequest"
    response_proto: "gibson.plugins.byte-identity.v1.SendMessageResponse"
```
...
declaration in `byte-identity.proto` plus the message names below.

## Step 2 — add proto messages

Edit `byte-identity.proto`:
...
- Proto message names in the manifest don't match the actual proto
  package + message in `byte-identity.proto`.
