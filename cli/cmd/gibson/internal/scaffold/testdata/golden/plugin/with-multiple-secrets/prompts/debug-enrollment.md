# Debugging byte-identity-multi enrollment
...
4. The CLI runs `capabilitygrant.Bootstrap → Discover → Register` and
   persists `~/.gibson/plugin/byte-identity-multi/host_key` (mode 0600).
...
need a fresh registration, delete `~/.gibson/plugin/byte-identity-multi/host_key`
and re-run.
...
```sh
ls -la ~/.gibson/plugin/byte-identity-multi/
```
