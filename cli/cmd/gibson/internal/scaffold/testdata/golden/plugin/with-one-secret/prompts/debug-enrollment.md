# Debugging byte-identity-secret enrollment
...
4. The CLI runs `capabilitygrant.Bootstrap → Discover → Register` and
   persists `~/.gibson/plugin/byte-identity-secret/host_key` (mode 0600).
...
need a fresh registration, delete `~/.gibson/plugin/byte-identity-secret/host_key`
and re-run.
...
```sh
ls -la ~/.gibson/plugin/byte-identity-secret/
```
