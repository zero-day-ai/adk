...
```sh
# 1. Generate Go bindings from byte-identity-multi.proto
make proto
...
# 4. Run (reads ~/.gibson/plugin/byte-identity-multi/host_key)
make run
```
...
```sh
docker build -t byte-identity-multi:0.99.0 .
docker run --rm \
  -e GIBSON_URL=https://api.zero-day.ai \
  -e GIBSON_PLUGIN_RUNTIME=pod \
  byte-identity-multi:0.99.0
```
