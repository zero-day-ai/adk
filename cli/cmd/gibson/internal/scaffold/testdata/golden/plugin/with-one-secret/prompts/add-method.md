# Adding a method to byte-identity-secret
...
`plugin.yaml`, `byte-identity-secret.proto`, and `main.go`.
...
spec:
  ...
  - name: SendMessage
    ...
    request_proto:  "gibson.plugins.byte-identity-secret.v1.SendMessageRequest"
    response_proto: "gibson.plugins.byte-identity-secret.v1.SendMessageResponse"
```
...
declaration in `byte-identity-secret.proto` plus the message names below.

## Step 2 — add proto messages

Edit `byte-identity-secret.proto`:
...
- Proto message names in the manifest don't match the actual proto
  package + message in `byte-identity-secret.proto`.
