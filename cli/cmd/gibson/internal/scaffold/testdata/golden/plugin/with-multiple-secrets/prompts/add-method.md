...
`plugin.yaml`, `byte-identity-multi.proto`, and `main.go`.
...
spec:
  ...
  - name: SendMessage
    ...
    request_proto:  "gibson.plugins.byte-identity-multi.v1.SendMessageRequest"
    response_proto: "gibson.plugins.byte-identity-multi.v1.SendMessageResponse"
```
...
declaration in `byte-identity-multi.proto` plus the message names below.

## Step 2 — add proto messages

Edit `byte-identity-multi.proto`:
...
- Proto message names in the manifest don't match the actual proto
  package + message in `byte-identity-multi.proto`.
